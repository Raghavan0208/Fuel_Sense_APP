...
import InsuranceLogEdit from './InsuranceLogEdit';
...
const InsuranceLogView = ({navigation, dashboardData}: Props) => {
  ...
  const groupedData = checkData
    ?.filter(item => item.insuranceCost > 0 && item.insuranceMainCategoryName) // ensure valid values
    ?.reduce((acc, item) => {
      const key = item.insuranceMainCategoryName;
      if (!acc[key]) {
        acc[key] = {insuranceMainCategoryName: key, insuranceCost: 0};
      }
      acc[key].insuranceCost += item.insuranceCost;
      return acc;
    }, {});
  ...
  const pieChartData = uniqueBrands.map((item: any, index: any) => {
    ...
    return {
      ...
      svg: {
        ...
        onPress: () =>
          setSelectedSlice({
            label: item.insuranceMainCategoryName,
            value: item.insuranceCost,
          }),
      },
      key: `pie-${index}`,
      insuranceMainCategoryName: item.insuranceMainCategoryName,
      color,
    };
  });
    ...
    if (Platform.Version >= 33) {
      const granted = await PermissionsAndroid.request(
        PermissionsAndroid.PERMISSIONS.READ_MEDIA_IMAGES,
        {
          ...
          message: 'App needs access to your images to upload photos',
      ...
      .then(res => {
        if (res.status === 200) {
          ...
          const filteredData = res.data.responseData.filter(
            (item: any) => item.insuranceMainCategoryName !== null,
          );
  ...
  const getAllInsuranceSpecificType = (
  ...
  const GetAllInsurancePrimaryDamageArea = (
    servicecategoryid: any,
    servicecomponentid: any,
    servicetypeid: any,
  ) => {
  ...
  const GetInsuranceLogExcel = async ({startDate, endDate, loginId}: any) => {
    CommonServices.getWithTripleQueryParam(
      'InsuranceLog',
      'GetInsuranceLogExcel',
      'startDate',
      ...
      .then(async res => {
        if (res?.status === 200) {
          ...
          const fileName = `FuelSense_InsuranceLog_Report_${moment(
            startDate,
                  ...
                  return (
                    <Box bg="emerald.700" px="4" py="2" rounded="sm" mb={5}>
                      <Text color={'#fff'} fontWeight={'bold'}>
                        Your InsuranceLog Excel report has been downloaded
                        successfully.
                      </Text>
  ...
  const onInsuranceLogPress = () => {
    ...
    CommonServices.post('InsuranceLog', 'Create', updateddata, countrycode)
      .then(res => {
        if (res.status == 200) {
          setservicelogmodelopen(false);
          Alert.alert('Success', 'Insurance Log Added Successfully');
          GetInsuranceLogChart(
            moment(pickDate).format('MM'),
            moment(pickDate).format('YYYY'),
    ...
    CommonServices.getWithTripleQueryParam(
      ...
      .then(async res => {
        if (res.status === 200 && res.data) {
          const fileName = `FuelSense_InsuranceLog_Report-${moment(
            startDate,
          ).format('YYYY-MM-DDTHH-mm-ss')}_${moment(endDate).format(
            'YYYY-MM-DDTHH-mm-ss',
          )}-${new Date().getTime()}`;
            ...
            if (!toast.isActive(id)) {
              toast.show({
                ...
                render: () => (
                  <Box bg="emerald.700" px="4" py="2" rounded="sm" mb={5}>
                    <Text color={'#fff'} fontWeight={'bold'}>
                      Your InsuranceLog PDF report has been downloaded
                      successfully.
                    </Text>
                    <Text color={'#fff'} fontWeight={'semibold'}>
                      Kindly check your Downloads folder.
    ...
    <SafeAreaView>
      ...
      <View style={{minHeight: screenHeight}}>
            ...
            <HStack
              ...
              {checkData.length > 0 ? (
                <VStack space={1} alignItems="flex-end" width={'50%'}>
                  <HStack space={4} justifyContent={'left'}>
                    <TouchableOpacity
                      onPress={GetInsuranceLogExcel.bind(null, {
            ...
            <View style={{flex: 1}}>
              <View
                ...
                <Stack
                  ...
                  <Center rounded="sm" shadow={'unset'}>
                    <Text style={styles.driveHero}>Total Insurance Cost</Text>
                    <Text
                      style={
                        styles.contentText
                      }>{`${countrysymbol} ${checkData[0]?.totalInsuranceCost.toFixed(
                      2,
                    )}`}</Text>
                  </Center>
                  <Divider mx="2" orientation="vertical" />
                          ...
                          {/* <View
                                                        style={[
                                                            ...
                                                            val.servicePriorityName?.toLowerCase() === 'high'
                                                                ? styles.high
                                                                : val.servicePriorityName?.toLowerCase() === 'medium'
                                                                    ? styles.medium
                                                                    : styles.low,
                                                        ]}
                                                    >
                                                        <Text style={styles.priorityText}>
                                                            {val.servicePriorityName?.trim() ? val.servicePriorityName : '--'}
                                                        </Text>
                                                    </View> */}
                          <HStack
                            justifyContent="space-between"
                            ...
                            <VStack space={1} width={'50%'}>
                              <Text style={styles.driveTile}>
                                ...
                                {val.insuranceMainCategoryName?.trim()
                                  ? val.insuranceMainCategoryName
                                  : '--'}
                              </Text>
                              <Text style={styles.driveTile}>
                                <Text style={styles.labelText}>
                                ...
                                {val.insuranceSubCategoryName?.trim()
                                  ? val.insuranceSubCategoryName
                                  : '--'}
                              </Text>
                              <Text style={styles.driveTile}>
                                <Text style={styles.labelText}>
                                ...
                                {val.insuranceSpecificTypeName?.trim()
                                  ? val.insuranceSpecificTypeName
                                  : '--'}
                              </Text>
                            </VStack>
                            <VStack
                              ...
                              <Text style={styles.driveTile}>
                                ...
                                {val.insurancePrimaryDamageAreaName}
                              </Text>
                              <Text style={styles.mileamt}>
                                <Text style={styles.labelText}>Cost: </Text>
                                {countrysymbol} {val.insuranceCost?.toFixed(2)}
                              </Text>
                              <Text style={styles.driveTile}>
                                <Text style={styles.labelText}>Date: </Text>
                                {val.insuranceLogDate
                                  ? moment(val.insuranceLogDate).format(
                                      'DD MMM, YYYY',
                                    )
                                  : '--'}
                              </Text>
                  ...
                  {selectedSlice.label !== '' && (
                    <View
                      ...
                      <Text style={{fontSize: 14}}>
                        Insurance Cost: {countrysymbol} {selectedSlice.value}
                      </Text>
                    </View>
                  )}
                  <PieChart
                      ...
                      {pieChartData?.map((item, index) => (
                        <View key={index} style={styles.legendItem}>
                          ...
                          <Text style={styles.legendText}>
                            {item.insuranceMainCategoryName ?? 'Unknown'}:{' '}
        ...
        <Modal isOpen={servicelogmodelopen} onClose={() => {
          ...
          <Modal.Content maxWidth="100%" w="100%" h="90%" maxHeight="90%">
            <Modal.CloseButton />
            <Modal.Header>Add Your Insurance Log</Modal.Header>
              ...
              <ScrollView  showsVerticalScrollIndicator={false}>
                ...
                {selectedTab === 'yes' && (
                  <>
                    ...
                    {/* Main Category */}
                    <View style={{ marginBottom: 12 }}>
                      <Text style={styles.locTitle}>Main Category *</Text>
                      <CustomDropDown
                    ...
                    {/* Sub Category */}
                    <View style={{ marginBottom: 12 }}>
                      <Text style={styles.locTitle}>Sub Category *</Text>
                      <CustomDropDown
                    ...
                    {/* Specific Type */}
                    <View style={{ marginBottom: 12 }}>
                      <Text style={styles.locTitle}>Specific Type *</Text>
                      <CustomDropDown
                    ...
                    {/* Primary Damage Area */}
                    <View style={{ marginBottom: 12 }}>
                      <Text style={styles.locTitle}>Primary Damage Area *</Text>
                      <CustomDropDown
                        data={servicePriorityArray}
                        placeholder="Primary Damage Area"
                    ...
                    {/* Insurance Cost */}
                    <View style={{ marginBottom: 12 }}>
                      <Text style={styles.locTitle}>Insurance Cost *</Text>
                      <CustomTextInput
                      ...
                      {isSubmitPressed && !serviceCost && (
                        <Text style={styles.errorText}>Insurance Cost is Required</Text>
                      )}
                    </View>

                    {/* Image Upload */}
                    <View style={{ marginBottom: 12 }}>
                      <Text style={styles.locTitle}>Insurance Note (image upload)</Text>
            ...
            <Modal.Footer>
              <Button.Group space={2}>
                ...
                <Button backgroundColor={'#000'} onPress={onInsuranceLogPress}>
                  Add Insurance Log
                </Button>
              </Button.Group>
            </Modal.Footer>
...
export default InsuranceLogView;
