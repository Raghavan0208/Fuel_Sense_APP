...
public static class InventLogger
{
    private const string hostingEnvironment = "Hosting:Environment";
    ...
    public static void LoggingConfiguration(IConfiguration configuration, string serviceName)
    {
        ...
        else if (Convert.ToBoolean(configuration["Logging:LogsWriteToFile"]))
        {
            ...
            Log.Logger = new LoggerConfiguration()
                            ...
                            .MinimumLevel.Override("Microsoft.AspNetCore", LogEventLevel.Warning)
                            .Enrich.FromLogContext()
                            .WriteTo.File(Path.Combine(loggingPath, string.Concat(serviceName, ".txt")), LogEventLevel.Debug,
        ...
        {
            string environmentName = Environment.GetEnvironmentVariable(hostingEnvironment);
            string loggingConnectionString = Convert.ToString(configuration["Logging:LogsDBConnectionString"]);
            string tableName = Convert.ToString(configuration["Logging:TableName"]);

            ColumnOptions customColumnOptions = new()
            {
            ...
            customColumnOptions.Store.Remove(StandardColumn.MessageTemplate);
            customColumnOptions.Store.Remove(StandardColumn.Properties);

            if (string.Equals(environmentName, HostingEnvironment_ENUM.Development.ToString())
                || string.Equals(environmentName, HostingEnvironment_ENUM.Staging.ToString()))
            {
                Log.Logger = new LoggerConfiguration()
                            .MinimumLevel.Debug()
                            .Enrich.FromLogContext()
                            .Enrich.WithProperty("ApplicationName", serviceName)
                            .WriteTo.MSSqlServer(loggingConnectionString, new MSSqlServerSinkOptions
                            {
                                TableName = tableName,
                                BatchPostingLimit = 100
                            }, restrictedToMinimumLevel: LogEventLevel.Information, columnOptions: customColumnOptions)
                            .WriteTo.Console(LogEventLevel.Information)
                            .CreateLogger();
            }
            else
            {
                Log.Logger = new LoggerConfiguration()
                            ...
                            .WriteTo.MSSqlServer(loggingConnectionString, new MSSqlServerSinkOptions
                            {
                                TableName = tableName,
                                BatchPostingLimit = 100,
                                BatchPeriod = new TimeSpan(0, 1, 0)
                            }, restrictedToMinimumLevel: LogEventLevel.Information, columnOptions: customColumnOptions)
                            .CreateLogger();
            }
        }
