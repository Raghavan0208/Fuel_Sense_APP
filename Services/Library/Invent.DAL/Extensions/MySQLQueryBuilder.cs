...
internal static class MySQLQueryBuilder
{
    ...
    internal static string GetInsertQuery(string tableName, dynamic item)
    {
        ...
        return string.Format("INSERT IGNORE INTO `{0}` ({1}) VALUES (@{2});SELECT LAST_INSERT_ID();", tableName, string.Join(",", columns), string.Join(",@", columns));
    }
    ...
    internal static string GetInsertBulkQuery(string tableName, dynamic item)
    {
        ...
        return string.Format("INSERT INTO `{0}` ({1}) into ##Inserted VALUES (@{2})", tableName, string.Join(",", columns), string.Join(",@", columns));
    }
    ...
    internal static string ConstructSelectInlineQuery<T>(this string tableName, string conditions = "")
    {
