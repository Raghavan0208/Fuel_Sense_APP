...
public class NotificationSenderService : BackgroundService
{
    ...
    public override async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        {
            Console.WriteLine("StarZero Notification Service started as a Console Application");
    ...
    private void ProcessPendingTasks()
    {
        ...
        if (pendingTasks?.Any() ?? false)
        {
            ...
            IEnumerable<SendNotificationCommand>? pendingSendSMSCommands = pendingTasks.Where(item => item.Id > 0 && canSendSMS == true) 
            ...
            IEnumerable<SendNotificationCommand>? pendingSendEmailCommands = pendingTasks.Where(item => item.Id > 0 && canSendEmail == true)
                   ?.Select(item => new SendNotificationCommand(item, this.appConfiguration));
