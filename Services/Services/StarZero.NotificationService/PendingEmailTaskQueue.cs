...
public class PendingEmailTaskQueue
{
    ...
    private ConcurrentQueue<ISendNotificationCommand> Queue
    {
        get
        {
            if (this.queue == null)
            {
                ...
                InventLogger.Information("Queue", "PendingEmail Task Queue Created");
            }
    ...
    public void Enqueue(IEnumerable<ISendNotificationCommand> indexUpdateCommands)
    {
        ...
        InventLogger.Information("Enqueue", string.Format("PendingEmailTask Queue length : {0}", this.Queue.Count));
    ...
    private void DequeueAndExecute()
    {
        ...
        try
        {
            ...
            InventLogger.Information("Email Sent");
        }
        catch (Exception ex)
        {
            InventLogger.Error(ex, "Exception Occurss while sending email");
        }
        finally
        {
