...
using StarZero.NotificationService.Extensions;
...
public class SendNotificationCommand : ISendNotificationCommand
{
    ...
    public void SendEmail(SmtpClient smtpClient)
    {
        ...
        {
            InventLogger.Fatal(ex, $"Email Sending Exception Occur:{this.NotificationTask.Id}");
        }
    }
    ...
    public void SendSMS(HttpClient httpClient)
    {
        ...
        if (!string.IsNullOrEmpty(apiurl) && canSendSMS)
        {
            ...
            bodyContent.root.message = this.appConfiguration.Value?.SMSConfiguration?.GlobalMessage ?? "StarZero Team";
            ...
            try
            {
                ...
                {
                    InventLogger.Warning($"SMS Sending Error Occured:{response.StatusCode}");
                }
            }
            catch (Exception ex)
            {
                InventLogger.Fatal(ex, $"SMS Sending Exception Occur:{this.NotificationTask.Id}");
            }
        } 
        else
        {
            InventLogger.Warning($"SMS Sending has been stopped or not working due to configiration, ApiUrl not available: {apiurl.Length <= 0} or EnableSMS: {canSendSMS}");
        }
    }
    ...
    private void AddEmailAttachments(MailMessage message)
    {
        ...
        InventLogger.Information($"The following attachment will send {this.NotificationTask?.Attachment} pending emailId {this.NotificationTask?.Id}");
        ...
        foreach (string attachment in attachmentFilePath)
        {
            ...
            {
                InventLogger.Information($"File is not exists {attachment} following pending emailId {this.NotificationTask?.Id}");
            }
    ...
    private void AddReportAttachments(MailMessage message)
    {
        ...
        foreach (string attachment in attachmentFilePath)
        {
            ...
            if (webPdf == null)
            {
                InventLogger.Information($"File is not exists in {filePath} following pending emailId {this.NotificationTask?.Id}");
            }
