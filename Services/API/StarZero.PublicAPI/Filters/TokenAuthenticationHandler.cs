...
public class TokenAuthenticationHandler : AuthenticationHandler<ServiceAPIAuthOptions>
{
    ...
    protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        if (Request.Headers.TryGetValue("Authorization", out StringValues authTokenValues))
        {
            ...
            if (authTokenValue.StartsWith(StarZeroConstants.ServiceAuthorizationHeaderScheme, StringComparison.OrdinalIgnoreCase))
            {
                tokenValue = authTokenValue.Substring(StarZeroConstants.ServiceAuthorizationHeaderScheme.Length).Trim();
            }
